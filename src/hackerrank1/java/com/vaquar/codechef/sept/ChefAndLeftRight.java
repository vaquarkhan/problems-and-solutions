...
/**
 * @see http://www.codechef.com/SEPT14/problems/CHEFLR/
 * 
 * 
 */
public class ChefAndLeftRight {
    ...
    public static BigInteger solve(char[] s) {
        BigInteger n = BigInteger.ONE;
        boolean even = false;
        for (char c : s) {
            if (even) {
                if (c == 'l') {
                    n = n.multiply(TWO).subtract(BigInteger.ONE);
                } else {
                    n = n.multiply(TWO).add(BigInteger.ONE);
                }
            } else {
                if (c == 'l') {
                    n = n.multiply(TWO);
                } else {
                    n = n.multiply(TWO).add(TWO);
                }
            }
            even = !even;
